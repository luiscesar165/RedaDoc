    ...
    <form action="{{route('service.update', $servicio->id)}}" id="form" method="POST" enctype="multipart/form-data">
      ...
      <div class="row">
        <div class="col-12 mb-4">
          <h5>Servicio</h5>
          <input class="form-control" id="input_title" type="text" name="service" value="{{$servicio->service}}" placeholder="Titulo" maxlength="191">
        </div> 
        <div class="col-12 mb-4">
          <h5>Imagen</h5>
          <input type="file" id="input_file" name="service_image">
        </div>
        <div class="col-12 mb-5">
          <a href="{{route('service.home')}}" type="button" class="btn btn-outline-danger px-4 mr-4">Cancelar</a>
          <input type="button" id="submit_button" class="btn btn-success px-5" value="Editar Servicio">
        </div>
      </div>
    </form>
  </div>
