...
<section>
  ...
  <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2">Editar Articulo</h1>
    ...
    <form action="{{route('blog.article.update', $article->id)}}" id="form" method="POST" enctype="multipart/form-data">
      @csrf
      <div class="row">
        <div class="col-12 mb-4">
          <h5>Titulo</h5>
          <input class="form-control" id="input_title" type="text" value="{{$article->title}}" name="title" placeholder="Titulo" maxlength="191">
        </div>
        <div class="col-12 mb-4">
          <h5>Descripción corta</h5>
          <input class="form-control" id="description" value="{{$article->description}}" maxlength="100" type="text" placeholder="Descripción corta" name="description">
        </div>
        <div class="col-12 mb-4">
          <h5>Contenido</h5>
          <textarea class="ckeditor" id="content" name="content" placeholder="Contenido">{{$article->content}}</textarea>
        </div>
        <div class="col-12 mb-4">
          <h5>Categorias</h5>
          <select class="form-control" name="category_id" id="category">
            ...
            @foreach($categorias as $category)
              <option value="{{$category->id}}" <?php if($category->id == $article->category->id) echo 'selected' ?> >{{$category->title}}</option>
            @endforeach
          </select>
        </div>
        <div class="col-12 mb-4">
        ...
        <div class="col-12 mb-5">
          ...
          <input type="button" id="submit_button" class="btn btn-success px-5" value="Actualizar Articulo">
        </div>
      </div>
    </form>
  </div>
